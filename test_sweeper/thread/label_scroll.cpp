...
void label_scroll::upateLabelRollingState()
{
    ...
    text_wpixel = fm.boundingRect(text() ); //以当前的字体格式为基础,计算字体的像素宽度
    #else
    text_wpixel = fm.width(text() ); //以当前的字体格式为基础,计算字体的像素宽度
    #endif

    if( (text_wpixel > this->height() ) && flag == 1 )// **长度或高度超出本身label的像素大小，则开启滚动***关键判断
    {
        left = 0; // 标志当前的像素滚动量

        #if QT_VERSION > QT_VERSION_CHECK(5,11,0)//根据官方文档说明，5.11后使用新的函数
        blank = " ";//空格
        blank_wp = fm.boundingRect(blank );//空格的像素宽度,方便后面计算是否到达末尾
        #else
        blank = " ";//空格
...
void label_scroll::timerEvent(QTimerEvent *e)
{
    if(e->timerId() == timerId && isVisible())
    {
        ...
        if((left + 20) > (text_wpixel + blank_wp) )// 表示到末尾了
            left = 1-( this->height() ); //重新添加,负数代表从最右边开始
...
void label_scroll::paintEvent(QPaintEvent *e)
{
    ...
    rc.setTop(rc.top() - left); //修改矩形 x轴, 由于left在不断变大，setLeft就在不断变小,(0,0)在左上角,固左移  change to top
